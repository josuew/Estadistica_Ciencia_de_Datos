...
```{r}
# readxl es una librería de R que nos permite leer el archivos con extensión .xlsx, esto nos ayuda para poder
# realizar lecturas de sheets (tabs en excel)
library(readxl)

# lubridate es una librería de R que nos permite trabajar con fechas, los cuales podemos extraer día, mes y año de alguna columna que contenga tipos de datos Date
library(lubridate)

# car es una librería de R que nos permite generar gráficos de densidad
library(car)
...
# Creamos una columna que se llama ANIO_SIGN_SINTOMAS que contiene el año del registro de la columna FECHA_SIGN_SINTOMAS
...
media_y_desviacion_estandar <- function (x){
  c(media = mean(x), desviacion_estandar = sd(x), median(x))
}
...
# Distribución de Edad por Entidad Federativa y Sexo
...
El estado de Guerrero presenta un mayor número de valores atípicos (outliers), lo que indica la existencia de registros correspondientes a personas en edades más avanzadas, es decir, adultos mayores. Por otro lado, Jalisco muestra los promedios y medianas de edad más altos (28 años para hombres y 32 para mujeres), mientras que Guerrero concentra las medianas más bajas para ambos sexos (16 años para hombres y 20 para mujeres), lo que sugiere que la población diagnosticada en esta entidad es, en general, más joven en comparación con las demás entidades federativas.

Además, al considerar la distribución por sexo, se observa que las medianas de edad para ambos sexos se encuentran entre los 16 y 32 años, lo que confirma que la mayoría de los casos de dengue ocurren en población joven.


# Distribución de la Edad por Entidad Federativa
...
En general, las distribuciones presentan una forma bimodal, con un primer pico pronunciado en edades jóvenes (entre 10 y 20 años) y un segundo pico menos marcado en edades adultas (aproximadamente entre 35 y 50 años).
...
Por su parte, Nayarit presenta un patrón similar a Michoacán, con la diferencia de que su segundo pico es ligeramente más alto, lo que indica un mayor número de casos en personas de edad avanzada en comparación con Michoacán.


# Distribución de edad en municipios con mayores casos por estado
...
Al igual que la distribución pasada, las distribuciones presentan una forma bimodal, con un primer pico pronunciado en edades jóvenes (entre 10 y 30 años) y un segundo pico menos marcado en edades adultas (aproximadamente entre 35 y 53 años).
...
Al analizar los gráficos de barras, se observa que septiembre es el mes con mayor número de casos registrados, superando los 15,000 casos, particularmente en Jalisco, lo cual coincide con su alta densidad poblacional en municipios como Guadalajara, como se indicó en análisis anteriores.

No obstante, Guerrero presenta un comportamiento atípico, mostrando su mayor registro de casos en enero, a diferencia de las demás entidades, que evidencian un incremento más gradual y estacional en sus registros. Esta diferencia sugiere patrones locales de transmisión que podrían estar influenciados por factores climáticos, demográficos o de vectorización específicos de cada estado.
