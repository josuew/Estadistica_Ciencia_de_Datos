...
sd(datos_ejemplo$Linea_A, na.rm = T)
datos_ejemplo
```
...
- a) Encuentre la probabilidad de que cierta cantidad de focos dure menos de 650 horas. P (X < 650) Sea X, la duracion de vida de los focos
```{r}
pnorm(650, 500, 100)
```
...
```{r}
(pnorm(780, 500, 100) - pnorm(650,500,100)) * 10000

```

- f) Si se eligen 1200 focos, ¿cuál es la probabilidad de que al menos 3 duren más de 780 horas?

$P(X_{1} > 780) | \mu = 500, \sigma=100)$ $X_{1}$ es la duracion de los focos > 780 horas
```{r}
peee <- pnorm(780, 500, 100, lower.tail = FALSE)
peee
```

$ P(X_{2} \geq 3 | p = 0.002, n = 1200) $ Con $ x_{2} $ el numero de focos que fallan en 1200 inspecciones

$ P(X_{2} \geq 3) = $ 1 - P(X_{2} < 3) = 1 - $P(X_{2} \leq 2)

```{r}
1 - pbinom(2, peee, size = 1200)
```
...
P(X_{1} < 650) $X_{1} es la duracion de los focos $X_{1} $ sigue una distribucion normal
P(16 \leq X_{2} \leq 19) $ $X{2}$ sigue una distribucion binomial
P(15 < x < 20) = P(X \leq 19) - P(X \leq 15) =
